...
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx {
    ...
    if(scaleWindow.windowEnd > scaleWindow.windowStart) {
        ...
        // Days
        NSDateComponents* cmpnts = [calendar components:NSCalendarUnitDay|NSCalendarUnitMonth|NSCalendarUnitYear fromDate:d];
        ...
        while (t <= t1) {
            NSDateComponents* c2 = [calendar components:NSCalendarUnitWeekday|NSCalendarUnitDay|NSCalendarUnitWeekOfMonth fromDate:d];
            NSInteger wd = c2.weekday;
            NSInteger md = c2.day;
            NSInteger wk = c2.weekOfMonth;
            CGFloat scale = 0.6;
            if(wd == fwd && displayCalendarUnits & NSCalendarUnitWeekOfMonth) {
                scale = 0.4;
            }
            if(md == 1) {
                scale = 0;
            ...
            char str[12] = "";
            if(displayCalendarUnits & NSCalendarUnitWeekday) {
                if(displayCalendarUnits & NSCalendarUnitDay) {
...
- (id) initWithFrame:(CGRect)frame
{
    ...
    if(self) {
        ...
        primaryLineUnits = NSCalendarUnitMonth;
        secondaryLineUnits = NSCalendarUnitWeekOfMonth;
        tertaryLineUnits = NSCalendarUnitDay;
...
- (void)drawLayer:(CALayer *)layer inContext:(CGContextRef)ctx {
    ...
    if(scaleWindow.windowEnd > scaleWindow.windowStart) {
        ...
        //int text = 0;
        while (t <= t1) {
            NSDateComponents* c2 = [calendar components:NSCalendarUnitWeekday|NSCalendarUnitDay fromDate:d];
            ...
            if(primaryLineUnits & NSCalendarUnitDay) {
                l1 = YES;
            }
            if(secondaryLineUnits & NSCalendarUnitDay) {
                l2 = YES;
            }
            if(tertaryLineUnits & NSCalendarUnitDay) {
                l3 = YES;
            }
            if(wd == fwd) {
                if(primaryLineUnits & NSCalendarUnitWeekOfMonth) {
                    l1 = YES;
                }
                if(secondaryLineUnits & NSCalendarUnitWeekOfMonth) {
                    l2 = YES;
                }
                if(tertaryLineUnits & NSCalendarUnitWeekOfMonth) {
                    l3 = YES;
                }
            }
            if(md == 1) {
                if(primaryLineUnits & NSCalendarUnitMonth) {
                    l1 = YES;
                }
                if(secondaryLineUnits & NSCalendarUnitMonth) {
                    l2 = YES;
                }
                if(tertaryLineUnits & NSCalendarUnitMonth) {
                    l3 = YES;
                }
            }
            /*if(YES) {
            ...
            if(color != nil) {
                if(true || gd.a != prevGridDash.a || gd.b != prevGridDash.b || color != prevGridColor) {
                    prevGridDash = gd;
                    prevGridColor = color;
                    if(gd.a != 0 || gd.b != 0) {
