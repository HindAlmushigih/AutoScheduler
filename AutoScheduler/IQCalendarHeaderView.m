    ...
    for(int i=0; i < 16; i++) {
        if(i < numItems) {
            if(itemLabels[i] == nil) {
                ...
                itemLabels[i].textAlignment = NSTextAlignmentCenter;
...
- (void)layoutSubviews
{
    ...
    CGSize textSize = [@"Gg" sizeWithFont:titleLabel.font constrainedToSize:CGSizeMake(1000, 1000) lineBreakMode:NSLineBreakByClipping];
    if(displayArrows) {
...
- (void)setTitleCalendarUnits:(NSCalendarUnit)units
{
    ...
    } else if(units & NSCalendarUnitMonth) {
...
- (void)setItemCalendarUnits:(NSCalendarUnit)units
{
    ...
    } else if(units & NSCalendarUnitDay) {
        [itemFormatter setDateFormat:@"EEE"];
    } else if(units & NSCalendarUnitWeekday) {
