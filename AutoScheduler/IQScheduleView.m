...
- (void) setStartDate:(NSDate*)s numberOfDays:(int)n animated:(BOOL)animated
{
    ...
    NSDateComponents* dc = [calendar components:NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay fromDate:s];
...
- (void) setStartDate:(NSDate*)s endDate:(NSDate*)e animated:(BOOL)animated
{
    ...
    NSDateComponents* dc = [calendar components:NSCalendarUnitDay|NSCalendarUnitHour fromDate:s toDate:e options:0];
    if(dc.day <= 0) {
...
{
    if(s == nil) s = [NSDate date];
    NSDateComponents* dc = [calendar components:NSCalendarUnitYear|NSCalendarUnitMonth|NSCalendarUnitDay|NSCalendarUnitWeekday fromDate:s];
    int diff, num;
    if(workdays) {
        diff = 2;
        num = 5;
...
- (void) setupCalendarView
{
    ...
    for(int i=1; i<= 23; i++) {
        ...
        hours[i].textAlignment = NSTextAlignmentRight;
